(function(){
  ...
  function parseRowDate(row){
    ...
    return {
      ...
      text: `${String(m).padStart(2,'0')}/${String(d).padStart(2,'0')}/${y}`
    };
  }

  // Find the most recent Full PM record
  ...
  if(!fullRow){
    alert('Could not find a prior Full PM record in the table.');
    return;
  }

  const fullPm = parseRowDate(fullRow);
  if(!fullPm){
    alert('Could not parse date from the Full PM record.');
    return;
  }

  const today = new Date();
  today.setHours(0,0,0,0);
  const compareDate = new Date(fullPm.date);
  compareDate.setHours(0,0,0,0);

  // Calculate elapsed days strictly since the LAST FULL PM
  const diffTime = today.getTime() - compareDate.getTime();
  const elapsedDaysSinceFull = Math.floor(diffTime / (1000 * 60 * 60 * 24));

  // Eligible for RECYCLE only if Full PM was done within the last 90 days
  const isRecycle = elapsedDaysSinceFull < 90;
  const statusText = isRecycle ? 'RECYCLE' : 'FULL PM';
  const statusColor = isRecycle ? '#4ade80' : '#e05252';
  const daysLeft = Math.max(0, 90 - elapsedDaysSinceFull);

  // 1-Year expiration based off the Full PM date
  const nextFullPmDate = new Date(fullPm.date);
  ...
  card.innerHTML = `
    ...
    <div id="pm_details_wrapper" style="display:none;">
      ...
      <div style="text-align:left;font-size:13px;line-height:1.8;color:#c4cbd4;margin-bottom:20px;">
        <div><span style="font-weight:700;color:#ffffff;">Last Full PM Date:</span> ${fullPm.text}</div>
        <div><span style="font-weight:700;color:#ffffff;">Days Since Full PM:</span> ${elapsedDaysSinceFull} day(s)</div>
        <div><span style="font-weight:700;color:#ffffff;">Days Left for Recycle:</span> ${daysLeft} day(s)</div>

(function () {
  ...
  html += `</div><div style="display:flex;gap:8px;"><button id="c_back" style="flex:1;padding:10px;background:#6c757d;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:13px;font-weight:bold;">← Back</button><button id="c_copy" style="flex:1.2;padding:10px;background:#28a745;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:13px;font-weight:bold;">Apply</button><button id="c_cancel" style="flex:1;padding:10px;background:#dc3545;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:13px;font-weight:bold;">Cancel</button></div>`;
  ...
  document.getElementById("c_copy").onclick = () => {
    ...
    let isWarranty = box.querySelector('.c_chk[data-warranty="1"]').checked;
    ...
    let costText = isWarranty ? "Parts and Labor covered under warranty" : `Parts and Labor total of $${checkedEls.reduce((sum, cb) => sum + parseFloat(cb.getAttribute("data-price") || 0), 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
